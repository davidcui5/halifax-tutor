...
public class StudentController {
    ...
    public String authorizeUser(@RequestBody Map<String, String> body) {
        String response = studentSetting.authorizeStudent(body.get("token"));
        return response;
    }

    @PostMapping(path = "/password")
    public String changePassword(@RequestBody Map<String, String> body) {
        String response = studentSetting.changePassword(body.get("token"), body.get("password"));
        return response;
    }

    @PostMapping(path = "/email")
    public String changeEmail(@RequestBody Map<String, String> body) {
        String response = studentSetting.changeEmail(body.get("token"), body.get("email"));
        return response;
    }

    @PostMapping(path = "/phone")
    public String changePhone(@RequestBody Map<String, String> body) {
        String response = studentSetting.changePhone(body.get("token"), body.get("phone"));
        return response;
    }

    @PostMapping(path = "/activation")
    public String checkActivationStatus(@RequestBody Map<String, String> body) {
        String response = studentSetting.checkActivationStatus(body.get("token"));
        return response;
    }

    @PostMapping(value = "/reactivation")
    public String resendActivateCode(@RequestBody Map<String, String> body) {
        String response = studentSetting.resendActivateCode(body.get("token"));
        return response;
    }
}
